...
def calculate_eigenvalues(matrix: list[list[float|int]]) -> list[float]:
    ...
    a, b, c = 1, -(matrix[0][0] + matrix[1][1]), matrix[0][0] * matrix[1][1] - matrix[0][1] * matrix[1][0]
    eigen_0, eigen_1 = (-b + math.sqrt(b * b - 4 * a * c)) / (2 * a), (-b - math.sqrt(b * b - 4 * a * c)) / (2 * a)
...
def transform_matrix(A: list[list[int|float]], T: list[list[int|float]], S: list[list[int|float]]) -> list[list[int|float]]:
    ...
    def matrix_multiplication(A: list[list[int|float]], B: list[list[int|float]]) -> list[list[int|float]]:
