...
def calculate_matrix_mean(matrix: list[list[float]], mode: str) -> list[float]:
    ...
    if mode == "row":
        ...
        means = [
            sum([row[i] for row in matrix]) / len(matrix) for i in range(len(matrix[0]))
        ]
...
def inverse_2x2(matrix: list[list[float]]) -> list[list[float]] | None:
    ...
    matrix_inverse = [
        [(1 / det_matrix) * elem for elem in row] for row in matrix_inverse
    ]
...
def calculate_covariance_matrix(vectors: list[list[float]]) -> list[list[float]]:
    ...
    def covariance(
        a: list[int | float], b: list[int | float], mean_a: float, mean_b: float
    ) -> int | float:
    ...
    for i in range(len(vectors)):
        for j in range(i, len(vectors)):
            covariance_matrix[i][j] = covariance(
                vectors[i], vectors[j], means[i], means[j]
            )
...
def softmax(scores: list[float]) -> list[float]:
    ...
    softmax_scores = [
        math.pow(math.e, scores[i] - max_score)
        / sum([math.pow(math.e, scores[j] - max_score) for j in range(len(scores))])
...
# Problem 35
def make_diagonal(x):
    # np.diag(x) is what we look for, but we'll do it another way
